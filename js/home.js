...
function renderCombo(){
  ...
  if(right){ right.innerHTML = HOME_DATA.comboRight.map(r=>(
    `<a href="${r.href}">
       <div>${r.title}</div>
...
function renderCollections(){
  ...
  row.innerHTML = HOME_DATA.collections.map(t=> `<a class="tag" href="#explore?collection=${encodeURIComponent(t)}">${t}</a>`).join('');
}
...
function renderGoods(){
  ...
  row.innerHTML = HOME_DATA.goods.map(g=>(
    `<a class="goods" href="${g.href}">
      <div class="ph"></div>
      <div class="meta"><div class="name">${g.name}</div><div class="price">${g.price}</div></div>
    </a>`)).join('');
}

document.addEventListener('DOMContentLoaded', ()=>{
  if (!document.querySelector('[data-page="home"]')) return;
  // 點搜尋導向 Explore（之後可換成真正搜尋）
